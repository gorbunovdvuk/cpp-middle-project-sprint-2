...
class format_string {
    ...
    static constexpr char get_specifier() {
        static_assert(!specifier.empty(), "Empty string");
        static_assert(specifier.front() == '{' && specifier.back() == '}', "Invalid placeholder");
        if constexpr (specifier == "{}") {
            return '\0';
        } else {
            constexpr auto stripped_specifier = specifier.substr(1, specifier.size() - 2);
            static_assert(stripped_specifier.front() == '%' && stripped_specifier.size() == 2, "Invalid placeholder");
    ...
    static constexpr placeholder internal_get_placeholder_position() {
        ...
        static_assert(end_position != data.size(), "Could not find matching '}'");
        return placeholder{
